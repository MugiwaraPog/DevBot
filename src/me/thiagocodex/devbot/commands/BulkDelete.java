        ...
        if (args[0].equalsIgnoreCase(DevBot.prefixMap.get(event.getGuild().getId()) + "clear")) {
            ...
            if (args[1].matches("^[\\p{Digit}]{1,3}$")) {
                ...
                if (num <= 100 && num > 1) {
                    ...
                    event.getChannel().sendMessage(embedMessage.info(event.getAuthor().getName() + ", sério? Você não precisa deletar em massa 1 mensagem. Exclua-a.")).queue();
                    return;
                } else {
                    event.getChannel().sendMessage(embedMessage.info(event.getAuthor().getName() + ", você não pode apagar mais de 100 mensagens de uma só vez.")).queue();
                    return;
                }
